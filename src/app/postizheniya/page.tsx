...
import { fetchUsers, fetchProgress } from '@/lib/api';
...
export default function PostizheniyaPage() {
  ...
  useEffect(() => {
    Promise.all([fetchUsers(), fetchProgress()])
      .then(([u, p]) => { setUsers(u); setActiveUser(u[0] ?? null); setProgress(p); })
      .catch(() => {});
  }, []);

  if (!progress || !activeUser) {
    return (
  ...
  const p = progress[activeUser.id] ?? { achievements: [] };
        ...
        {ACHIEVEMENTS.map(a => {
          ...
          return (
            <div
              ...
              style={{
                background: done ? `${activeUser.color}14` : '#F8FAFC',
                border: `1.5px solid ${done ? activeUser.color + '44' : '#E2E8F0'}`,
                opacity: done ? 1 : 0.55,
              }}
            >
              <span className="text-3xl" style={{ filter: done ? 'none' : 'grayscale(1)' }}>{a.emoji}</span>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-slate-800 text-sm">{a.title}</div>
                <div className="text-xs text-slate-500">{a.desc}</div>
                <div className="text-xs font-semibold mt-0.5" style={{ color: activeUser.color }}>+{a.xp} XP</div>
              </div>
              {done && <span className="text-emerald-500 text-xl">✓</span>}
            </div>
          );
        })}
      </div>
