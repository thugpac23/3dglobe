...
interface Country  { name: string; capital: string; flag: string }
interface FamousPerson { name: string; country: string; hint: string }
interface FactQuestion { prompt: string; correct: string; wrong: string[]; category: string }
interface Question { category: string; prompt: string; correct: string; choices: string[] }
...
const GEOGRAPHY_FACTS: FactQuestion[] = [
  { prompt: 'Коя е най-дългата река в света?', correct: 'Нил', wrong: ['Амазонка', 'Янцзъ', 'Мисисипи'], category: 'География' },
  { prompt: 'Кой е най-високият планински връх в света?', correct: 'Еверест', wrong: ['К2', 'Килиманджаро', 'Монблан'], category: 'География' },
  { prompt: 'Кой е най-големият океан на Земята?', correct: 'Тихи океан', wrong: ['Атлантически', 'Индийски', 'Арктически'], category: 'География' },
  { prompt: 'Кой е най-голямата пустиня в света?', correct: 'Сахара', wrong: ['Гоби', 'Намиб', 'Калахари'], category: 'География' },
  { prompt: 'На кой континент е Бразилия?', correct: 'Южна Америка', wrong: ['Северна Америка', 'Африка', 'Азия'], category: 'География' },
  { prompt: 'Колко континента има на Земята?', correct: '7', wrong: ['5', '6', '8'], category: 'География' },
  { prompt: 'Кой е най-малкият континент?', correct: 'Австралия', wrong: ['Европа', 'Антарктида', 'Южна Америка'], category: 'География' },
  { prompt: 'Коя страна е с най-голяма площ в света?', correct: 'Русия', wrong: ['Канада', 'Китай', 'САЩ'], category: 'География' },
  { prompt: 'Кой е най-дълбокият океан в света?', correct: 'Тихи океан', wrong: ['Атлантически', 'Индийски', 'Арктически'], category: 'География' },
];

const ANIMAL_FACTS: FactQuestion[] = [
  { prompt: 'Кое животно е най-бързото на сушата?', correct: 'Гепард', wrong: ['Лъв', 'Кон', 'Щраус'], category: 'Животни' },
  { prompt: 'Кое е най-голямото животно на Земята?', correct: 'Син кит', wrong: ['Слон', 'Жираф', 'Акула'], category: 'Животни' },
  { prompt: 'Кое животно спи правостоящо?', correct: 'Жираф', wrong: ['Слон', 'Зебра', 'Хипопотам'], category: 'Животни' },
  { prompt: 'Кое е националното животно на Австралия?', correct: 'Кенгуру', wrong: ['Коала', 'Ехидна', 'Вомбат'], category: 'Животни' },
  { prompt: 'Колко крака има октопода?', correct: '8', wrong: ['6', '10', '4'], category: 'Животни' },
  { prompt: 'Кое животно има най-дълъг език спрямо тялото си?', correct: 'Хамелеон', wrong: ['Жираф', 'Мравояд', 'Гущер'], category: 'Животни' },
  { prompt: 'Кое животно може да живее без вода най-дълго?', correct: 'Камила', wrong: ['Слон', 'Пустинна мишка', 'Кенгуру'], category: 'Животни' },
];
...
function makeQuestion(): Question {
  ...
  if (type === 0) {
    ...
    return { category: '🏙️ Столици', prompt: `Каква е столицата на ${correct.flag} ${correct.name}?`, correct: correct.capital, choices: shuffle([correct.capital, ...wrong.map(c => c.capital)]) };
  }
  if (type === 1) {
    const correct = pickRandom(COUNTRIES);
    const wrong = shuffle(COUNTRIES.filter(c => c.name !== correct.name)).slice(0, 3);
    return { category: '🗺️ Обратно', prompt: `В коя страна се намира столицата ${correct.capital}?`, correct: `${correct.flag} ${correct.name}`, choices: shuffle([`${correct.flag} ${correct.name}`, ...wrong.map(c => `${c.flag} ${c.name}`)]) };
  }
  if (type === 2) {
    const person = pickRandom(FAMOUS_PEOPLE);
    const wrongCountries = shuffle(COUNTRIES.filter(c => c.name !== person.country)).slice(0, 3).map(c => `${c.flag} ${c.name}`);
    const correctCountry = COUNTRIES.find(c => c.name === person.country);
    const correctLabel = correctCountry ? `${correctCountry.flag} ${person.country}` : person.country;
    return { category: '🌟 Известни личности', prompt: `${person.name} (${person.hint}) е от коя страна?`, correct: correctLabel, choices: shuffle([correctLabel, ...wrongCountries]) };
  }
  if (type === 3) {
    const fact = pickRandom(GEOGRAPHY_FACTS);
    return { category: `🌍 ${fact.category}`, prompt: fact.prompt, correct: fact.correct, choices: shuffle([fact.correct, ...fact.wrong]) };
  }
...
export default function QuizGame({ activeUser }: { activeUser: UserType }) {
  ...
  // Reset to idle when the user switches
  ...
  const answer = useCallback((choice: string) => {
    ...
    if (correct) { sounds.quizCorrect(); setScore(s => s + 1); setXpEarned(x => x + 5); }
    ...
    <div>
      ...
      {(phase === 'playing' || phase === 'answered') && question && (
        <div>
          ...
          <div className="rounded-2xl p-6 mb-5 text-center shadow-sm" style={{ background: `${USER_COLOR[activeUser]}10`, border: `2px solid ${USER_COLOR[activeUser]}30` }}>
            ...
            <div className="mt-5 flex items-center gap-4">
              <div className="shrink-0 flex flex-col items-center gap-1">
                {avatarUrl ? (
                  ...
                  <div className="flex items-center justify-center rounded-2xl text-4xl" style={{ width: 80, height: 106, background: `${USER_COLOR[activeUser]}18` }}>
                    {isCorrectAnswer ? '🎉' : '🤔'}
                  </div>
                )}
                <span className="text-base">{isCorrectAnswer ? '🎉' : '🤔'}</span>
              </div>
              <button onClick={nextQuestion} className="flex-1 py-3 rounded-2xl font-bold text-white transition-all" style={{ background: USER_COLOR[activeUser] }}>
      ...
      {phase === 'done' && (
        <div className="text-center py-8">
          ...
          <p className="text-slate-500 mb-2">{score === TOTAL_ROUNDS ? 'Перфектно!' : score >= TOTAL_ROUNDS / 2 ? 'Много добре!' : 'Продължавай да учиш!'}</p>
          {xpEarned > 0 && (
